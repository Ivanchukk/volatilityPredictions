...
from data_processing.googleCloud import getBlobsNames
...
def main(recordNewData, market ,minutesTorecord, recordstartDate, bucket_name='volatility1'):
    ...
    vs = Visualisation()
    vs.plot_predictions(preds_list, real_vals_list)
    vs.diff_percent(diff_percent)
    vs.dtal(results)
...
# main1('KNCUSDT')
def recordNewData(market ,minutesTorecord, recordstartDate, candleKind='1m'):
    bucket_name='volatility1'
    print("im in new data creator" * 10, candleKind)
    FolderManger(market, candleKind).createFolder()
    blob_name = market.lower() #need to create folders in cloud for candle type
    gs = GgCloud(market, candleKind)
    dl = DataLoader(market, minutesTorecord, candleKind)
    print("Starting Recording process")
    path = dl.record_candles_to_pkl_full(recordstartDate, minutesTorecord) #Maybe minutesToRecord not relevant here because it in the class
    print("recording "* 10)
    print(blob_name)
    print(path)
    gs.upload_to_bucket(blob_name, path)

def recordexistingMarket(market ,minutesTorecord, candleKind):
    # Create Folder
    FolderManger(market, candleKind).createFolder()
    ...
    blob_name = market.lower()
    prefix = blob_name
    dl_dir = os.path.join(os.getcwd(), f'data_processing/rawDataFolder/{market}/{candleKind}')
    gs = GgCloud(market, candleKind)
    gs.downloadFiles(prefix, dl_dir)
    dl = DataLoader(market, minutesTorecord, candleKind)
    # dl.load_full_pkl(dl_dir)

    path = dl.record_candles_to_pkl_full()
    vs = Visualisation()
    latestData = pd.read_pickle(path)
    vs.plotLine(pd.to_datetime(latestData['date']), latestData.open.astype(float))
    gs = GgCloud(market, candleKind)
    print("recording "* 10)
    print(blob_name)
    print(path)
...
def run_predictions(market, candleKind='1m', predPeriods=1, neurons=16, activefunc='tanh', numEpochs=50, batchSize=68, optimizer='adam'):
    bucket_name = 'volatility1'
    blob_name = f"{market.lower()}/{candleKind}"
    gs = GgCloud(market, candleKind)
    dl = DataLoader(market,"nan MinutesToRecord" ,candleKind)
    ...
    dataV = VolatlityCalculation(dataP).infinite_sequence()
    fea = FeatureExtractor(dataV, predPeriods).createSequences()
    ...
    diff_percent, preds_list, real_vals_list, resultMAE, resultMAEstd = Model(X_train, y_train, X_test, y_test, yscaler, predict_field, features, sequence_length, neurons, activefunc, numEpochs, batchSize, optimizer).evaluate()
    now = datetime.now()
    print(preds_list)
    print(real_vals_list)
    results = pd.DataFrame([preds_list, real_vals_list])
    ...
    return [resultMAE, resultMAEstd, preds_list, real_vals_list]
    # vs.dtal(results)

def justPrint():
    return ["asd", "qwe"]

run_predictions("KNCUSDT", '1m')
